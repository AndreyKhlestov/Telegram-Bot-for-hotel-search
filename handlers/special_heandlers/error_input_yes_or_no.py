...
@bot.message_handler(state=[UserState.ask_photo, UserState.confirm])
@logger.catch()
def error_confirm(message: Message) -> None:
    """Функция для оповещения пользователя о неверных действиях при ожидании от нажатия кнопок да или нет"""
